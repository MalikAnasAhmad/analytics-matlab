...
fft_y_window = fft_y_window.*(abs(fft_y_window)<800);
...
% ylim([0 2000])
hold on
plot(status(no_of_sample_for_window_size:fft_y_len)*80,'r')
hold off
ylabel('frequency amplitude')
xlabel('recording index')
...
fft_z_window = fft_z_window.*(abs(fft_z_window)<800);
...
% ylim([0 2000])
hold on
plot(status(no_of_sample_for_window_size:fft_z_len)*200,'r')
hold off
ylabel('frequency amplitude')
xlabel('recording index')
